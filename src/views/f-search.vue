  ...
  <div class="about">
		...
		<div class="sear">
			...
			<ul v-if='val'class="list_s">
				<li v-for="i,index in arr_s"  style="height:20px;background:#ccc;">{{index+1}}.{{i}}</li>
			</ul>
		</div>
		<!-- 搜索数据输出-->
...
import { constants } from "crypto";
		...
		export default({
			...
			methods:{
				a(){
						this.axios.get('https://elm.cangdu.org/v1/cities/'+this.id).then(data => {
						this.arr=data.data
							})
				},
				submits(){
						...
						this.data_list=''
							...
							this.axios.get('https://elm.cangdu.org/v1/pois?city_id='+this.id+'&keyword='+this.val).then(data => {
