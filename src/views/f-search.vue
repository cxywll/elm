  ...
  <div class="about">
  	    <div class="title" >
			<span style="margin-right: 120px;">
				<router-link to='/' style='color: white; font-size:30px'>
					&lt;
				</router-link>
			</span>
			<span style="font-weight: 600;font-size:16px;">{{arr.name}}</span>
			<span style="float: right;">
				<router-link to='/' style='color: white;font-size: 14px;'>
					切换城市
				</router-link>
			</span>
		</div>
		...
		<div class="sear">
			...
			<ul v-if='val' class="list_s">
				<li v-for="(i,index) in arr_s"  style="{height:20px;background:#ccc;}" :key="index">{{index+1}}.{{i}}</li>
			</ul>
		</div>
		<ul v-if="data_list"  style="list-style: none;min-height:60px;">
            <router-link to="/Hello" v-for="(i,index) in data_list" :key="index">
...
import { constants } from "crypto";
		...
		export default({
			...
			methods:{
				a(){
						this.$http.get('https://elm.cangdu.org/v1/cities/'+this.id).then(data => {
						this.arr=data.data
							})
				},
				submits(){
						...
						this.data_list=''
							...
							this.$http.get('https://elm.cangdu.org/v1/pois?city_id='+this.id+'&keyword='+this.val).then(data => {
