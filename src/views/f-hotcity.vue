<template>
	<div id="app">
		...
		<div class="hot">   
			...
			<div class="list" v-for="(i,index) in hotcity" :key="index">
		...
		<div class="type">
			<div v-for="(s,key,index) in soarr" :key="index">
				<h4 class="letter">{{key}}</h4>
				<div class="list" v-for="(i,index) in s" :key="index">
					<router-link :to='/f-search/+i.id'>
						{{i.name}} 
					</router-link>
				</div>
			</div>
...
<script type="text/javascript">
	export default {
		...
		methods: {
			hot() {
				this.$yhttp.get('https://elm.cangdu.org/v1/cities?type=hot').then(data => {
					this.hotcity = data.data
				})
			},
			all() {
				this.$yhttp.get('https://elm.cangdu.org/v1/cities?type=group').then(data => {
